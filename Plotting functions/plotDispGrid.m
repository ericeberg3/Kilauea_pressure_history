...
for row = 1:nPlot
    ...
    for col = colRange
        ...
        if col == 1 || isSinglePlot
             ...
             % ylabel(ax, GPSNameList{iSite}, 'Rotation',90, 'FontSize',18, 'FontWeight','bold', 'HorizontalAlignment','center', 'Color', label_color);
...
if hasSDH
    ...
    for col = sdhLoop
        ...
        plot(ax, ts, m, '-', 'Color', fitCol,  'LineWidth',1.6);
        xline(ax, 0, 'k:','LineWidth',1);
        ax.FontSize = 16; %grid(ax,'on');
        
        if col == 1 || isSinglePlot
            % ylabel(ax,'SDH (µrad)', 'Rotation',90, 'FontSize',18, 'FontWeight','bold', 'HorizontalAlignment','center');
        else
            ax.YTickLabel = [];
        end
        
        xlabel(ax,'','FontSize',18);
        box(ax,'on'); ax.LineWidth = 1.5;
        ylim(ax, yLimsSDH);
        hold(ax,'off');
    end
    ...
    if ~isSinglePlot
        ...
        legend(axLeg,'show', 'Location','north', 'Orientation','vertical', 'Box','off', 'FontSize', 28);
    end
end
