...
coh_threshold = 0.3;
coh_mask = min_cc_asc < coh_threshold;          % true = incoherent pixel to discard
cum_disp_asc_masked = cum_disp_asc;
cum_disp_asc_masked(coh_mask) = NaN;

% Crop the image
unw_crop = cum_disp_asc_masked(1:new_rows, 1:new_cols);
...
for bs = blockSizes'
    ...
    blocks = qtgetblk(cum_disp_asc_masked, S, bs);
    if ~isempty(blocks)
        % Find indices (upper left corners) of blocks with size bs in S
        [r, c] = find(S == bs);
        for k = 1:length(r)
            % Define the block indices in cum_disp_asc_masked
            block = cum_disp_asc_masked(r(k):r(k)+bs-1, c(k):c(k)+bs-1);
...
for i = 1:numel(coh_list)
    nexttile;
    fr = quadtree_by_coh(cum_disp_asc, min_cc_asc, coh_list(i), tol, B);
...
exportgraphics(thresh, './asc_mask_grid.png', 'Resolution', 200);
