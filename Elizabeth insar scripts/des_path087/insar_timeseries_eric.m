...
look_desc = [mean(east(:)), mean(north(:)), mean(up(:))];
% Example plot: showing East look vector. It doesn't vary a ton in the
% study area. 
figure; imagesc(flongitudes, flatitudes, east); colorbar;
 ...
 % Note these look vectors are also for 5/29/18 acquisition. It may be
...
% test_mask = mean(cc_array,3)>0.35;
% 
% cc_avg_desc = mean(cc_array, 3);
% displacement_desc = squeeze(displacement(:,:,end));
% 
% 
% doPhaseVideo = 1; 
% if doPhaseVideo == 1
...
for bs = blockSizes'
    ...
    for k = 1:length(r)
        ...
        % If the mean coherence is below the threshold, mark the entire block as invalid
...
save desc_cov_data.mat X Y latitudes longitudes avg_cc_asc cum_disp_rad blocks insarx insary
%% Displacement video 
doDispVideo = 0; 
if doDispVideo == 1
