...
fid=fopen("S1B_IW_RAW__0SDV_20180529T161523_20180529T161556_011138_0146D9_6C3B.lookvector",'r');
...
avglook_desc = squeeze( lookvector(:, iLon, iLat) );  
save look_desc.mat avglook_desc;
% avglook_desc = [mean(east(:)); mean(north(:)); mean(up(:))];
...
cum_disp_desc = squeeze(displacement(:,:,end)).';
avg_cc_desc = mean(cc_array, 3);
% max_cc_desc = max(cc_array, 3);
min_cc_desc = min(cc_array, [], 3);
bot10_cc_desc = prctile(cc_array, 10, 3);
bot30_cc_desc = prctile(cc_array, 30, 3);
...
%% Quadtree downsample
% [insaru_desc, blocks_desc] = quad_downsample(cum_disp_desc, avg_cc_desc', longitudes, latitudes, coast_new);

% Adjust tol based on data’s noise and variability characteristics
tol = 0.1;

% Get the original dimensions
[nrows, ncols] = size(cum_disp_desc);
...
cc_tests(2, :, :) = min_cc_desc(1:new_rows, 1:new_cols);
...
clf;
subplot(1,2,1);
imagesc(longitudes, latitudes, cum_disp_desc);
hold on;
...
cumDisp_looked = cumDisp * avglook_desc;
...
quiver(-155.22, 19.47, avglook_desc(1), avglook_desc(2), 1e-1, "filled", "LineWidth", 4)
...
%         insaru_pred_HMM(i, j) = spheroid(taiyi_parameters(1:8), [xy_temp(1); xy_temp(2); 0], 0.25, 3.08*10^9)' * avglook_desc;
%         insaru_pred_SC(i, j) = spheroid(taiyi_parameters(9:end), [xy_temp(1); xy_temp(2); 0], 0.25, 3.08*10^9)' * avglook_desc;
%     end
% end
% 
...
% filtered_resampled_test = downsample_coh(coh_list(i),nan_list(j), min_cc_desc, resampled, S);
% imagesc(longitudes, latitudes, resampled);
% set(gca, 'YDir', 'normal');
% hold on;
% colormap default;
...
writematrix(data_out, 'descending.txt', 'Delimiter', 'tab');
...
cum_disp_rad = cum_disp_desc*2*pi / lambda;
save desc_cov_data.mat X Y latitudes longitudes avg_cc_desc cum_disp_rad blocks insarx insary
